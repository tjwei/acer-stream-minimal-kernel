...
static void acer_hs_early_suspend(struct early_suspend *h);
static void acer_hs_early_resume(struct early_suspend *h);
#endif

static struct acer_hs_data {
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static void acer_headset_delay_set_work(struct work_struct *work)
{
	...
	if (state) {
		...
		if (get_hs_type_state()) {
			...
			if (get_butt_adc() > -1 && get_butt_adc() < 6) {
		...
		schedule_delayed_work(&set_hs_butt_wq, 100);
...
void hs_amp(bool enable)
{
	if(enable) {
		...
		hr->headsetOn = false;
		tpa2018_headset_switch(0);
	}
}

void hs_mic_en(int enable)
{
	if (!enable) {
		if (gpio_get_value(hr->det)) {
			gpio_set_value(hr->mic_bias_en,1);
		} else {
			gpio_set_value(hr->mic_bias_en,enable);
		}
	} else {
		gpio_set_value(hr->mic_bias_en,enable);
	}
	pr_info("[Audio HS] hs_mic_en = %d \n",gpio_get_value(hr->mic_bias_en));
}
...
static int acer_hs_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(hr->irq, hs_det_irq,
			  IRQF_TRIGGER_RISING, "hs_detect", NULL);
	if (ret < 0)
	{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	acer_hs_data.early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	acer_hs_data.early_suspend.suspend = acer_hs_early_suspend;
	acer_hs_data.early_suspend.resume = acer_hs_early_resume;
	register_early_suspend(&acer_hs_data.early_suspend);
#endif
...
static void acer_hs_early_suspend(struct early_suspend *h)
{
	ACER_HS_DBG("acer_hs_early_suspend +++\n");
	pr_debug("[ACER_HS] %s ++ entering\n", __FUNCTION__);
	pr_debug("[ACER_HS] %s -- leaving\n", __FUNCTION__);
	ACER_HS_DBG("acer_hs_early_suspend --- \n");
}

static void acer_hs_early_resume(struct early_suspend *h)
{
	ACER_HS_DBG("acer_hs_early_resume +++ \n");
	pr_debug("[ACER_HS] %s ++ entering\n", __FUNCTION__);
	//Re-detect headset detect pin.
	if (((switch_get_state(&hr->sdev) != 0) && (gpio_get_value(hr->det) == 0)) ||
		((switch_get_state(&hr->sdev) == 0) && (gpio_get_value(hr->det) != 0))) {
		schedule_work(&short_wq);
		schedule_delayed_work(&set_hs_state_wq, 20);
	}
	pr_debug("[ACER_HS] %s -- leaving\n", __FUNCTION__);
