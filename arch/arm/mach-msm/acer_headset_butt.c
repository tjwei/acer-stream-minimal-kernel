...
static enum hrtimer_restart button_event_timer_func(struct hrtimer *data)
{
	...
	if(!(gpio_get_value(hr->dett)==1 && gpio_get_value(hr->mic)==1))
...
int get_butt_adc(void)
{
	...
	while (count < 3) {
		adc_tmp = rpc_call_func();

		if (adc_tmp > -1 && adc_tmp < 6) break;
		else if(adc_tmp > 24 && adc_tmp < 41) break;
		else if(adc_tmp > 53 && adc_tmp < 79) break;
...
static void rpc_call_work(struct work_struct *work)
{
	...
	if(state){
		while(count<3){
			...
			if(adc_tmp > -1 && adc_tmp < 6) break;
			else if(adc_tmp > 29 && adc_tmp < 41) break;
			else if(adc_tmp > 59 && adc_tmp < 79) break;
		...
		if(adc_reply > -1 && adc_reply < 6) // ADC Range: 0~5
		{
			key_code = KEY_MEDIA;
		}
		else if(adc_reply > 24 && adc_reply < 41) // ADC Range: 25~40
		{
			key_code = KEY_VOLUMEUP;
		}
		else if(adc_reply > 53 && adc_reply < 79) // ADC Range: 54~78
		{
		...
		if( key_code != prekey ){
			input_report_key(hr->input, prekey, 0);
		}
...
static int rpc_call_func(void){
	...
	memset(&req_packet, 0x09, sizeof(req_packet));
