...
static int s_in_tolerance_flag = 0;
...
*/
int smb380_calc_new_offset(smb380acc_t orientation, smb380acc_t accel, unsigned short *offset_x,
									unsigned short *offset_y, unsigned short *offset_z)
{
	short old_offset_x, old_offset_y, old_offset_z;
	short new_offset_x, new_offset_y, new_offset_z;

	unsigned char calibrated =0;
	s_in_tolerance_flag = 0;
	...
	if ((accel.x > 7) | (accel.x < -7)) {           /* does x axis need calibration? */
		pr_info("x[%d] needs calibrate...\n", accel.x);
		if ((accel.x <14) && accel.x > 0)              /* check for values less than quantisation of offset register */
			new_offset_x= old_offset_x -1;
		else if ((accel.x >-14) && (accel.x < 0))
			new_offset_x= old_offset_x +1;
		else
			new_offset_x = old_offset_x - (accel.x/14);  /* calculate new offset due to formula */
	...
	if ((accel.y > 7) | (accel.y<-7)) {            /* does y axis need calibration? */
		pr_info("y[%d] needs calibrate...\n", accel.y);
		if ((accel.y <14) && accel.y > 0)             /* check for values less than quantisation of offset register */
			new_offset_y= old_offset_y -1;
		else if ((accel.y >-14) && accel.y < 0)
			new_offset_y= old_offset_y +1;
		else
			new_offset_y = old_offset_y - accel.y/14;  /* calculate new offset due to formula */
	...
	if ((accel.z > 7) | (accel.z<-7)) {            /* does z axis need calibration? */
		pr_info("z[%d] needs calibrate...\n", accel.z);
		if ((accel.z <14) && accel.z > 0)             /* check for values less than quantisation of offset register */
			new_offset_z= old_offset_z -1;
		else if ((accel.z >-14) && accel.z < 0)
			new_offset_z= old_offset_z +1;
		else
			new_offset_z = old_offset_z - (accel.z/14);/* calculate new offset due to formula */
	...
	if( (accel.x < 15 && accel.x > -15) &&
		(accel.y < 15 && accel.y > -15) &&
		(accel.z < 15 && accel.z > -15))
	{
		s_in_tolerance_flag = 1;
	}
...
int smb380_calibrate(smb380acc_t orientation, int *tries)
{
	...
	if ( (*tries>0 && *tries < ltries) || s_in_tolerance_flag) {
		if (*tries==0) {
			pr_info("The calibration offset is still in the tolerance.");
		}
