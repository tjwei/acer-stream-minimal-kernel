...
* The resale and/or use of products are at the purchaser own risk and his own responsibility. The
...
int smb380_init(smb380_t *smb380)
{
	...
	comres += smb380_set_interrupt_mask(0);
	/* set bandwidth and g-range */
	comres += smb380_set_range(SMB380_RANGE_2G);
	comres += smb380_set_bandwidth(SMB380_BW_25HZ);
...
int smb380_get_offset(unsigned char xyz, unsigned short *offset)
{
	...
	*offset = data;
	if(xyz == 2){
		comres = p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, (SMB380_OFFSET_X_MSB__REG), &zdata[0],3);
		*offset |= (zdata[2]<<2);
	}
	else {
...
int smb380_selftest(unsigned char st)
{
	int comres;
	unsigned char data;
	unsigned char orig_bandwidth, orig_range;
	smb380acc_t acc;
	unsigned int vector;
	int res;
	if (3==st) {
		/* store now range and bandwidth setting */
		smb380_get_bandwidth(&orig_bandwidth);
		smb380_get_range(&orig_range);
		/* start vector self test */
		res = -1;
		smb380_set_bandwidth(SMB380_BW_25HZ);
		smb380_set_range(SMB380_RANGE_4G);
		smb380_pause(50);
		smb380_read_accel_xyz(&acc);
		vector = (acc.x * acc.x) + (acc.y * acc.y) + (acc.z * acc.z);
		pr_info("[BMA150] vector self test: acc.x:[%d], acc.y:[%d], acc.z:[%d], vector:[%d]\n", acc.x, acc.y, acc.z, vector);
		/* 0.55g < absolute sum < 1.7g */
		if ((vector > 4956) && (vector < 47350)) {
			res = 0;
			pr_info("[BMA150] vector self test succeed!\n");
		}
		else {
			pr_err(">>> [BMA150] vector self test failed!\n");
		}
		/* restore original range and bandwidth setting */
		smb380_set_bandwidth(orig_bandwidth);
		smb380_set_range(orig_range);
		return res;
	}
