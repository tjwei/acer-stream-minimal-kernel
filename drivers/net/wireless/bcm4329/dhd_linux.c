...
_dhd_set_multicast_list(dhd_info_t *dhd, int ifidx)
{
	...
	ret = dhd_prot_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len);
	if (ret < 0) {
		DHD_TRACE(("%s: set promisc %d failed\n",
		           dhd_ifname(&dhd->pub, ifidx), ltoh32(allmulti)));
	}
}
...
dhd_open(struct net_device *net)
{
	...
	printf("%s:Not Force start if ifconfig_up gets called before START command\n", __FUNCTION__);
...
//define in include/linux/mmc/host.h
//0:Stop SDIO HW IRQ, 1:Start SDIO HW IRQ
...
dhd_module_cleanup(void)
{
	...
	printf("%s: (%d, %d)\n", __FUNCTION__, g_IsWifiModuleLoaded, deny_dhd_open);
